...
public class LessonController {
    ...
    public ResponseEntity<Page<LessonModel>> getAllLessons(@PathVariable(value = "moduleId")UUID moduleId,
                                                           ...
                                                           @PageableDefault(page = 0, size = 10, sort = "lessonId", direction = Sort.Direction.ASC)
                                                               Pageable pageable){
        return ResponseEntity.status(HttpStatus.OK).body(lessonService.findAllByModuleId(SpecificationTemplate.findLessonsByModuleId(moduleId).and(spec), pageable));
    }
